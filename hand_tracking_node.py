            ...
            # '''
                if num_tips == 1 and len(self.boxls) > 0:
                    ...
                    if isstatic:
                        ...
                        elif self.donetracting:
                            self.donetracting = False
                            temp_result = []
                            rng = np.array([tiptract])
                            im = np.zeros([480, 640])
                            cv2.fillPoly(im, rng, 255)
                            for cx, cy in self.surfacels:
                                if im[cy, cx] == 255:
                                    temp_result.append((cx, cy))
                            tiptract = []
                        ...
                        if len(tiptract) > 1:
                            if distant(tiptract[0], tips[0]) < distant(tiptract[0], tiptract[1]):
                                tiptract.append([tips[0][0], tips[0][1]])
                                isstatic = True
